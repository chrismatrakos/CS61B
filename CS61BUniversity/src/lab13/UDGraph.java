...
public class UDGraph {
	...
	private void paths(UDGraph g, int length, int j, int startindex) {

		if (length == 0) {
			g.addEdge(startindex, j);
			return;
		}
		for (int i = j; i < vertices; i++) {
			for (int z = 0; z < vertices; z++) {
				if (this.adjMatrix[i][z] == true) {
					paths(g, length - 1, z, startindex);
				}
			}
			return;// if it does not find an edge in this row of matrix dont
					// continue to next!!!
		}
	}
