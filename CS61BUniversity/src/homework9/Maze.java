...
public class Maze {
	...
	public Maze(int horizontalSize, int verticalSize) {
		...
		int wallNum = (((horiz-1)*vert)+((vert-1)*horiz));
		DisjointSets dset = new DisjointSets(horiz*vert);
		Wall[] aw = new Wall[wallNum];
		int c = 0;
		if (vert > 1) {
		...
		// for (Wall t : aw) {
		// System.out.println(t.x + " " + t.y + t.type);
		// }

		random = new Random();
		// System.out.println(index);
		for (int index=wallNum-1;index>0;index--) {
		...
		for (i = 0; i <wallNum; i++) {
			...
			if (temp.type==1) {
				...
				if (dset.find(x+(y*horiz)) != dset.find(x+(y*horiz)+1)) {
					//System.out.println(temp.x + " " + temp.y);
					dset.union(dset.find(x+(y*horiz)),dset.find(x+(y*horiz)+1));
					vWalls[x][y] = false;
					
				}
			}
			if (temp.type==0) {
				if (dset.find(x+(y*horiz)) != dset.find(x+(y*horiz)+horiz)) {
					dset.union(dset.find(x+(y*horiz)),dset.find(x+(y*horiz)+horiz));
					hWalls[x][y] = false;
				}
	...
	public static void swap(Wall a[], int i, int j) {
		Wall temp = a[i];
		a[i] = a[j];
		a[j] = temp;
	}
